...
import { SchedulePayment } from "@/components/home/schedule";
import { DistributorDeploy } from "@/components/home/distributor";
import { DistributePayment } from "@/components/home/distribute-payment";

export default function Home() {
	const [account, setAccount] = useState<Account>();
	const [showAddSessionKey, setShowAddSessionKey] = useState(false);
	const [showScheduler, setShowScheduler] = useState(false);
	const [showDistributor, setShowDistributor] = useState(false);

	const renderScreen = () => {
		if (account) {
			if (showAddSessionKey) {
				return (
					<AddSessionKey
						account={account}
						onComplete={() => {
							setShowAddSessionKey(false);
